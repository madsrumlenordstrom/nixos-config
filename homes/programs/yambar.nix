{ inputs, outputs, lib, config, pkgs, ...}:
{
  programs.yambar = {
    ...
    settings = with config.colorScheme.palette; let
      ...
      scaledInt = n: lib.strings.toInt (lib.head (lib.strings.splitString "." (lib.strings.floatToString ((lib.head (lib.filter (m: m.primary) config.monitors)).scale * n))));

      # Main monitor where bar will be put
      monitor = (lib.head (lib.filter (m: m.primary) config.monitors)).name;
    ...
    in {
      bar = {
        ...
        border = {
          color = "${base04}ff";
          margin = lib.mkDefault (scaledInt 6);
          bottom-margin = lib.mkDefault (scaledInt 0);
          width = scaledInt 1;
        };
            ...
            battery = {
              ...
              content.map = {
                ...
                in {
                  ...
                  "state == full" = [ { string.text = lib.lists.last (charging); } ];
                };
              };
            };
          }
